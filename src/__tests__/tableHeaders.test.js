...
describe('Headers configuration', () => {
  ...
  describe('transactionsHeaders', () => {
    ...
    test('calls formatDate for purchaseDate', () => {
      ...
      expect(dataAggregator.formatDate).toHaveBeenCalledWith(row.purchaseDate);
    });
  ...
  test('should cover formatDate, price and reward render functions', () => {
    ...
    const customerName = transactionsHeaders[1].render(row);
    const date = transactionsHeaders[2].render(row);
    const price = transactionsHeaders[4].render(row);
